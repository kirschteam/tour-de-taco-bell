...
**Stretch goal — the full original menu.** The 38 variant items the dedupe cut (bacon/steak breakfast versions, supreme tacos, zero-sugar and dirty sodas, the black-bean line, etc.) are back in the app as optional ⭐ stretch items: they don't count toward the core goal, the completion %, or the plan, but they log like any other item and feed a separate stretch tracker on the Race tab (all 104 items eaten, +11,375 cal, +~$137 if the group clears the whole board). Verified against the live store menu 2026-08-30 — only still-sold variants made the cut (the 10 discontinued ones, e.g. Dragonfruit Berry Refresca and the Crispy Chicken line, stay out).
...
- **Menu**: all 104 items (66 core + 38 ⭐ stretch, stretch rows dashed) grouped by category with calories/macros, searchable with a ⭐ Stretch filter, tap to claim/log an item for a rider.
